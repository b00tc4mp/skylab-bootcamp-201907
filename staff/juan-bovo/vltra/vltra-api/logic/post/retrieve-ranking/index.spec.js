...
describe('logic - retrieve ranking', () => {
    ...
    it('should succeed on correct id', async () => {
        ...
        expect(ranking[1].id.toString()).to.equal(postId3)
        expect(ranking[2].id.toString()).to.equal(postId)
    })
    ...
    it('should fail if there are no posts', async () => {
        ...
        try {
            ...
            expect(error.message).to.equal(`there are no post to retrieve`)
    }
    })
    ...
    it('should fail if there are no posts with votes to retrieve', async () => {
        ...
        try {
            ...
            expect(error).to.exist
            expect(error.message).to.equal(`there are no post with votes to retrieve`)
    }
    })
