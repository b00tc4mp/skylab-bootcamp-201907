...
describe('logic - create post', () => {
    ...
    it('should succeed creating a post on correct data', async () => {
        const result = await createPost(author, title, body)
    ...
    it('should fail if post has already been published', async () => {
        ...
        const firstTry = await Post.create({author, title, body, date, comments, votes})
            try{
                await createPost('5d74c89d6b50d08960730147', 'title', body, date, comments, votes)
    ...
    it('should fail if body is larger than 2000 chars', async () => {
        ...
        try {
            await createPost(author, title, largeBody)
            throw new Error('should not reach this point')
        } catch (error) {
            expect(error).to.exist
    ...
    it('should fail if title is larger than 100 chars', async () => {
        ...
        try {
            await createPost(author, largeTitle, body)
            throw new Error('should not reach this point')
        } catch (error) {
            expect(error).to.exist
    ...
    it('should fail on undefined author', () => 
        expect(() => 
            createPost(undefined, title, body)
