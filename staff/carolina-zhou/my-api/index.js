...
client.connect()
    .then(() => {
        ...
        app.delete('/user/:id', jsonBodyParser, (req, res) => {
            ...
            try {
                ...
                logic.unregisterUser(id)
                    .then(user => res.json({ message: 'user correctly deleted', user }))
                    .catch(({ message }) => res.status(404).json({ error: message }))
            } catch ({ message }) {
                res.status(404).json({ error: message })
            }
        ...
        app.put('/user/:id', jsonBodyParser, (req, res) => {
            const { body: { email, name, surname, password }, headers: { authorization } } = req
            const token = authorization.slice(authorization.indexOf(' ') + 1)

            try {
                jwt.verify(token, secret)
                logic.updateUser(email, name, surname, password)
