...
let score = [
  { names: "DoomGuy", points: 7, time: "xxx", fails: 20, status: 0 },
  { names: "LaVaNe", points: 18, time: "xxx", fails: 9, status: 0 },
  { names: "Marvin", points: 25, time: "xxx", fails: 2, status: 0 },
  { names: "Mrs-Pots", points: 26, time: "xxx", fails: 0, status: 0 },
  { names: "El_Perchas", points: 19, time: "xxx", fails: 8, status: 0 },
  { names: "Anna", points: 13, time: "xxx", fails: 14, status: 0 },
  { names: "Splinter", points: 1, time: "xxx", fails: 26, status: 0 }
];
...
function accesGame() {
  ...
  if (toogle == true) {
    //Si la seccion 'main' es cargada por segunda vez.
    ...
    // si es la primera vez que se carga el juego.
...
function checkAnswer() {
  ...
  if (Object.values(arrCopy).every(value => value.status == 1) == true) {
    ...
    addToScore(
      window.user,
      countHits,
      [minutes.innerHTML + ":" + seconds.innerHTML],
      countFails
    );
    allTrue = true;
    message.innerHTML = "";
    ...
    setTimeout(function() {
      ...
      userScore.innerHTML = `<p><span class="user-score-color">${
        window.user
      }</span>, has acertado un total de <span class="user-score-color">${countHits}</span> preguntas<br/>en <span class="user-score-color">${
        minutes.innerHTML
      }:${
        seconds.innerHTML
      }</span> min. <span class="user-score-color">${msg}</span></p>`;
    }, 1000);
...
function pasapalabra() {
  ...
  let letters = document.getElementById(`${arrCopy[counter].letter}`);
  let changeValueDescription = {
    info: `Object index: ${counter} with letter: ${Object.values(
      arrCopy[counter].letter
    )} has been moved to the last position`,
    status: 1
  };
...
function resetGame() {
  ...
  document.getElementById("show-question").innerHTML =
    "<p>Dale al <b>Start</b> para empezar</p>";
...
function showScore() {
  ...
  result.forEach(
    (value, index) =>
      (userScore.innerHTML += `<tr class=${
        index % 2 == 0 ? "even" : "odd"
      } id=${value.status == 1 ? "current-user" : "nothing"}><td>${index +
        1}</td><td>${value.names}</td><td>${value.points}</td><td>${
        value.fails
