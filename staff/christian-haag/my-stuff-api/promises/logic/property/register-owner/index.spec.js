...
describe('logic - add property', () => {
    ...
    beforeEach(() => {
        ...
        return User.deleteMany()
            .then(() => { return Property.deleteMany() })
            .then(() => Promise.all([User.create({ name, surname, email, password }), Property.create({ address, sqm, yearOfConstruction, cadastre, mortgage }), User.create({ name, surname, email, password })]))
            .then(([user, property, user2]) => {
                ownerId = user.id
                propId = property.id
                ownerId2 = user2.id
            })
    ...
    it('schould register a new owner', () => {
        return logic.registerOwnerProperty(propId, ownerId)
            .then(propertyId => {
    ...
    it('schould register a second owner', () => {
        return logic.registerOwnerProperty(propId, ownerId)
            .then(() => {
                return logic.registerOwnerProperty(propId, ownerId2)
                    .then(propertyId => {
                        return Property.findOne({ _id: propertyId })
                            .then(prop => {
    ...
    it('should fail if owner not exist', () => {
        return logic.registerOwnerProperty(propId, '5d5d5530531d455f75da9fF9')
            .catch(({ message }) => {
                expect(message).to.equal('No permission to add new owner')
            })
    })

    it('should fail if owner already exist', () => {
        return logic.registerOwnerProperty(propId, ownerId2)
            .catch(({ message }) => {
    ...
    it('should fail if property not exist', () => {
        return logic.registerOwnerProperty('5d5d5530531d455f75da9fF9', ownerId)
            .catch(({ message }) => {
