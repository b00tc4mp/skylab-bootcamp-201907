...
describe('logic - add property', () => {
    ...
    beforeEach(() => {
        ...
        return User.create({ name, surname, email, password })
            .then((user) => {
                ownerId2 = user.id
                return Promise.all([logic.registerOwnerProperty(propId1, ownerId1), logic.registerOwnerProperty(propId1, ownerId2)])
            })
    ...
    it('property should have two owners', () => {
        return Property.find({ owners: { $all: [ownerId1, ownerId2] } })
            .then(result => {
    ...
    it('schould delete one owner', () => {
        return logic.unregisterOwnerProperty(propId1, ownerId1)
            .then(() => {
                return Property.findOne({ _id: propId1 })
            })
            .then(property => {
