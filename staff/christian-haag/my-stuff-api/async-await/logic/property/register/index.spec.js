...
describe('logic - add property', () => {
    ...
    it('should fail if user does not exist', async () => {
        try {
            await logic.registerProperty('5d5d5530531d455f75da9fF9', address, sqm, yearOfConstruction, cadastre, mortgage)
        } catch ({ message }) {
            expect(message).to.equal('user 5d5d5530531d455f75da9fF9 does not exist')
        }
    })

    it('schould add property only if user exists', async () => {
        await logic.registerProperty(id, address, sqm, yearOfConstruction, cadastre, mortgage)
    ...
    describe('logic - add another property', () => {
        ...
        it('should fail if property already exists', async () => {
            try {
                await logic.registerProperty(id, address, sqm, yearOfConstruction, cadastre, mortgage)
            } catch ({ message }) {
