...
describe('logic - update property', () => {
    ...
    beforeEach(async () => {
        ...
        const [user, property] = await Promise.all([User.create({ name, surname, email, password }), Property.create({ address, sqm, yearOfConstruction, cadastre, mortgage })])
    ...
    it('should match with registered data', async () => {
        const result = await Property.findOne({ _id: propertyId }).lean()
    ...
    it('should update property data', async () => {
        await logic.updateProperty(propertyId, propUpdate)
