...
const { registerUser, authenticateUser, retrieveUser, updateUser, unregisterUser, retrieveFavorites, toggleFavorite, registerWish, uploadImage } = require('./user')
...
router.post('/user/dog/:dogId', [tokenMiddleware, jsonBodyParser], toggleFavorite)
router.post('/user/wish', [tokenMiddleware, jsonBodyParser], registerWish)
router.post('/user/image/:id', tokenMiddleware, uploadImage)
...
router.post('/user/chat/:chatId', [tokenMiddleware, jsonBodyParser], updateChat)
