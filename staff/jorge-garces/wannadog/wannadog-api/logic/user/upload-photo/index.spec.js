...
const { database, models: { User, Dog } } = require('generisad-data')
const { random } = Math
const fs = require('fs')
...
describe.only('logic - upload image', () => {
    ...
    beforeEach(async () => {
        ...
        const ad = await Dog.create({ title, description, price, location, date, owner: id })
        adId = ad.id
        image = fs.createReadStream('./test/smiley.jpg')
    })
    it('should succeed on correct image', async () => {
        const result = await uploadImage(id, adId, image)
        expect(result).not.to.exist
        const ad = await Dog.findById(adId)
        expect(ad).to.exist
