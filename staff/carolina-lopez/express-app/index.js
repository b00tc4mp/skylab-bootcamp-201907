    ...
    try {
        if (userId && token)
            Promise.all([
                ...
                .then(([user, ducks]) => res.send(Html(`${Header(user.name, query, lang)}${DuckResults(ducks, lang)}`)))
                .catch(error => { throw error })
        else
            logic.retrieveFavDucks(undefined, undefined, query)
                .then(ducks => res.send(Html(`${Header(undefined, query, lang)}${DuckResults(ducks, lang)}`)))
    } catch (error) {
        throw error
    }
})
