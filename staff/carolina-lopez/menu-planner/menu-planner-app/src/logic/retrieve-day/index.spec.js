...
describe('logic - retrieve day', () => {
    ...
    it("should retrieve a day on correct data", async () => { 
        const day = await logic.retrieveCurrentDay(userId)

        expect(day).toBe()
        
        expect(day.breakfast.id.toString()).toBe(id1)
        expect(day.lunch.id.toString()).toBe(id2)
        expect(day.snack.id.toString()).toBe(id3)
        expect(day.dinner.id.toString()).toBe(id4)
    })

    it("should fail on non existant id", async () => {
      id = "5d740a810a6041a5ae918901"
      try {
        await logic.retrieveCurrentDay(id)
      } catch({message}) {
        expect(message).toBe()
    ...
    it("should fail if query is not a string", () => {
        expect(() => logic.retrieveCurrentDay(123)).toThrow(Error, "userId with value 123 is not a string")
    })

    it('should fail on empty id', () =>
        expect(() =>
            logic.retrieveCurrentDay('')
        ).toThrow('userId is empty or blank')
    )

    it('should fail on undefined id', () =>
        expect(() =>
            logic.retrieveCurrentDay(undefined)
