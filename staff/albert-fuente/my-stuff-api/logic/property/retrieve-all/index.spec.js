...
describe('logic - retrieve all properties', () => {
    ...
    beforeEach(async()=> {
        ...
        password = `123-${Math.random()}`
         ...
         const user=await User.create({ name, surname, email, password })
            ...
            const newProperty2 =await new Property({ address: address2, m2: m2v2, year: year2 , cadastre: cadastre2})
            propertyId2 = newProperty2.id
            newProperty2.owners.push(userId)
            return await Promise.all([newProperty.save(), newProperty2.save()])
        
    })
    it('should succeed on correct data', async() => {
        const properties= await logic.property.retrieveAll(userId)
    ...
    it('should fail on incorrect data',async () => {
        try{
            await logic.property.retrieveAll('3123414')
        }catch (error){
