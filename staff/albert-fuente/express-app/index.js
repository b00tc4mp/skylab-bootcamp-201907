    ...
    try {
        if (userId && token)
            Promise.all([
                ...
                .then(([user, ducks]) => res.send(Html(`${Header(user.name, query, SEARCH, SIGN_IN, SIGN_UP, SIGN_OUT, FAVORITE)}${DuckResults(ducks)}`)))
                .catch(error => { throw error })
        else
            logic.searchDucks(undefined, undefined, query)
                .then(ducks => res.send(Html(`${Header(undefined, query, SEARCH, SIGN_IN, SIGN_UP, SIGN_OUT, FAVORITE)}${DuckResults(ducks)}`)))
    } catch (error) {
        throw error
    }
})
    ...
    try {
        if (userId && token)
            Promise.all([
                ...
                .then(([user, duck]) => res.send(Html(`${Header(user.name, query, SEARCH, SIGN_IN, SIGN_UP, SIGN_OUT, FAVORITE)}${DuckDetail(duck)}`)))
                .catch(error => { throw error })
        else
            logic.retrieveDuck(undefined, undefined, duckId)
                .then(duck => res.send(Html(`${Header(undefined, query, SEARCH, SIGN_IN, SIGN_UP, SIGN_OUT, FAVORITE)}${DuckDetail(duck)}`)))
    } catch (error) {
        throw error
    }
})
