...
describe('logic - search articles', () => {
    before(() => database.connect(DB_URL_TEST))
    
    let query, articleId1, articleId2
    ...
    it('should succeed on correct data', async () => {
        ...
        const articles = await searchArticles(query)
                ...
                expect(articles[0].id).to.equal(articleId1)
                ...
                expect(articles[1].id).to.equal(articleId2)
...
/*     it('should fail if the ref is not a number', () => 
        expect(() => retrieveArticle('abc').to.throw(`article[0] with ref 'abc' does not exist`))
    ) */
