        ...
        if(rosco.status === 0){
          document.getElementById("pregunta").innerHTML = rosco.question
            ...
            if(preguntaLetra === rosco.answer){
                ...
                rosco.status = 2;
                console.log(`Fallaste!, la respuesta es: ${rosco.answer}`);
                falladas++;
            }
        }
...
function aciertosFallos(){
    if(preguntas.every(item => item.status === 1 )){
            ...
            console.log(`MUY BIEN! HAS GANADO 1 MILLON DE EUROS`)
        }
    else{
        console.log(`Has acertado ${acertadas}, y has fallado ${falladas} que lástima.`)
    }
}
...
function welcome() {
    ...
    let bienvenida = console.log(`Hoy jugamos con ${concursante}, con un bote de 1 MILLON DE EUROS! VAMOS ALLÁ!!!!`);
}
