...
export default withRouter(function ({ history, onLogout }) {
    ...
    const { user, selectedPoll, setSelectedPoll } = useContext(Context)

    
     useEffect(() => {
        (async () => {
            
            const res = await logic.listAll('5d70f41b7d4edc12334851db')
    ...
    <h2 className="pollcategoryname"><b>Expired</b></h2>
    {polls && <ul>
